...
public class BindMobileServiceImpl implements BindMobileService {
	...
	public ResultBean vaildateUnbindPhone(String memberId, String phone, String payPwd, String smsCode) {
		...
		//校验短信验证码
		int vcode = smsService.verifyCode(ModuleTypeEnum.UNBINDPHONE.getCode(), pm.getPhone(), smsCode);
		...
		try {
			...
			return new ResultBean("BM0025","校验解绑手机异常");
		} catch(Exception e){
			e.printStackTrace();
			log.error(e.getMessage());
			return new ResultBean("BM0025","校验解绑手机异常");
	...
	public ResultBean modifyPhone(String memberId, String phone, String smsCode) {
		try {
			...
			int vcode = smsService.verifyCode(ModuleTypeEnum.BINDPHONE.getCode(), pm.getPhone(), smsCode);
			SmsValidateEnum valEnum =SmsValidateEnum.fromValue(String.valueOf(vcode));
			if(valEnum != SmsValidateEnum.SV1){
				return new ResultBean("BM0001", valEnum.getMsg());
			}	
			//获取会员所属合作机构ID
			PoMemberBean memberByPhone = memberService.getMemberByPhoneAndCoopInsti(phone, pm.getInstiId());
			if(memberByPhone!=null){
				return new ResultBean("BM0026", "手机号已经被注册 ");
			}
			//更新会员手机号 t_member 
			if(!memberOperationService.modifyPhone(memberId, phone)){
				return new ResultBean("BM0027", "更新手机号失败 ");
			}
		} catch (Exception e) {
			e.printStackTrace();
			log.error(e.getMessage());
			return new ResultBean("BM0028","修改解绑手机异常");
		}
		return new ResultBean(true);
	}
	...
	public ResultBean vaildateBankCardForModifyPhone(String memberId, long bindId, String cardNo, String certNo,
		...
		try {
			...
			if(!memberBankCard.getIdnum().equals(certNo)){
				return new ResultBean("BM0029", "身份证号错误");
			}
			//校验支付密码
			MemberBean member = new MemberBean();
			member.setLoginName(pm.getLoginName());
			...
			e.printStackTrace();
			log.error(e.getMessage());
			return new ResultBean("BM0030","校验绑卡信息和支付密码异常");
		} catch(Exception e){
			e.printStackTrace();
			log.error(e.getMessage());
			return new ResultBean("BM0030","校验绑卡信息和支付密码异常");
